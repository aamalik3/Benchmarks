...
int encrypted_save (u32 DDR_OFFSET , u32 OFFSET);
...
//		print("PCAP Frame Read-back ");
		if(print_on == true){
			...
			int Status;
			Xil_DCacheDisable(); /*Caches must be disabled otherwise PCAP read back fails*/
			Xil_ICacheDisable();
...
//			usleep(100000); /*It works with this delay*/
//			usleep(90000);
		if(print_on == true){
...
//	u32 Statusreg;
	...
	if (!(Status & 0x01)) {
		...
		/*Zynq TRM UG585 v1.4 Page 1660, Register PCAP_CLK_CTRL */
		XDcfg_WriteReg(0xF8000000,0x168,(Status | 0x01));/*Original Setting in the Example*/
//		XDcfg_WriteReg(0xF8000000,0x168,0x001);/*Setting (0x001->0x701) divides the PCAP Clock by a factor of 7,i.e 100/7 = approx. 14Mhz*/
//		XDcfg_WriteReg(0xF8000000,0x168,0x701);/*Setting (0x001->0x701) divides the PCAP Clock by a factor of 7,i.e 100/7 = approx. 14Mhz*/
...
void PrintBitStream(u32 address,bool print_on)
{
	...
	/*First 100 Frames*/
	for(int i=0; i <100; ++i){
		ctxbuf[i*4+0]= readback_buffer[i+101]>>24 & 0xFF;
		ctxbuf[i*4+1]= readback_buffer[i+101]>>16 & 0xFF;
		ctxbuf[i*4+2]= readback_buffer[i+101]>>8  & 0xFF;
		ctxbuf[i*4+3]= readback_buffer[i+101]>>0  & 0xFF;
	}
	/*Frame #101-- First 32bits*/
		ctxbuf[400]= readback_buffer[201]>>24 & 0xFF;
		ctxbuf[401]= readback_buffer[201]>>16 & 0xFF;
		ctxbuf[402]= readback_buffer[201]>>8  & 0xFF;
		ctxbuf[403]= readback_buffer[201]>>0  & 0xFF;
	/*Frame #101-- Remaining Padding Bits*/
...
int save_bitstream (u32 DDR_OFFSET , u32 OFFSET , bool BRAM_FLIP)
{
	for(u32 DRAM_Addr=0; DRAM_Addr <101; DRAM_Addr++)
	{
	#ifdef SWAP_ON
...
int encrypted_save(u32 DDR_OFFSET , u32 OFFSET)
{
	for(u32 DRAM_Addr=0; DRAM_Addr <104; DRAM_Addr++)
	{
		u32 data= ctxbuf[DRAM_Addr*4+0]<<24 | ctxbuf[DRAM_Addr*4+1]<<16 |ctxbuf[DRAM_Addr*4+2]<<8|ctxbuf[DRAM_Addr*4+3];
		/*
		if ((DRAM_Addr==4||DRAM_Addr==14|| DRAM_Addr==24|| DRAM_Addr==34|| DRAM_Addr==44|| DRAM_Addr==55|| DRAM_Addr==65|| DRAM_Addr==75|| DRAM_Addr==85|| DRAM_Addr==95) && BRAM_FLIP){
			Xil_Out32(((DDR_OFFSET+OFFSET)+ (4*DRAM_Addr)), data & BRAM_MASK);
     	}
		else{
			Xil_Out32(((DDR_OFFSET+OFFSET)+ (4*DRAM_Addr)), data);
		}
		*/
		Xil_Out32(((DDR_OFFSET+OFFSET)+ (4*DRAM_Addr)), data);
	}
