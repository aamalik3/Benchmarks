...
module tb_top_verilator
    #(parameter INSTR_RDATA_WIDTH = 32, //128
      parameter RAM_ADDR_WIDTH = 22, //22,
    ...
    logic         	     cpu_cont_en; //added
    logic        	     trigger_o; //added
    logic                   cpu_stalled; //added
    logic [10:0]             cnt_stall; //added
        ...
        if($value$plusargs("firmware=%s", firmware)) begin
            if($test$plusargs("verbose"))
                $display("[TESTBENCH] %t: loading firmware %0s ...",
                         $time, firmware);
            $readmemh(firmware, cv32e40x_tb_wrapper_i.dp_ram_i.mem);
	...
	if (~rst_ni) begin
     ...
     assign cpu_cont_en = (cnt_stall == 10);
    ...
    cv32e40x_tb_wrapper_i
        (.clk_i          ( clk_i          ),
         ...
         .cpu_cont_en    ( cpu_cont_en    ), //added
         .trigger_o      ( trigger_o      ), //added
         .cpu_stalled    ( cpu_stalled    ), //added
